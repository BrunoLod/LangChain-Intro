...
# Importando a biblioteca itemgetter, a qual me permite 'pegar'
# valores presentes num dicionário Python. 
...
# Quando eu coloco em template string - {cidade} - a cidade, significa que
# estou capturando o valor da chave cidade, presente na saída da primeira cadeia, 
# que retorna um dicionário. Caso eu quisésse capturar também o motivo, bastaria
# escrever : {motivo}
...
# Criando um template voltado a produzir um formato de resposta final do bot. 

modelo_resposta_final = ChatPromptTemplate.from_messages(
    [
        ("ai", "Espaços culturais e artísticos que você tem que visitar {espacos_culturais_artisticos}"), 
        ("ai", "Sugestão de viagem para a {cidade}"), 
        ("ai", "Livraria, cafés e bares recomendados {espacos_recomendados}"), 
        ("ai", "Combine as informações anteriores em 3 parágrafos coerentes.")
    ]
)
...
cadeia_4 = modelo_resposta_final | llm | StrOutputParser()
...
cadeia = (cadeia_1 | {"espacos_culturais_artisticos" : cadeia_2, 
                    "espacos_recomendados": cadeia_3, 
                    "cidade": itemgetter("cidade")} | cadeia_4)
